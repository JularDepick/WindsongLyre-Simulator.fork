...
使用按键字母编写乐谱，括号 `()` 内为和弦同时弹奏，`|` 后接数字控制节奏。逗号 `,` 表示1个单位休止，句号 `.` 表示4个单位休止。输入不区分大小写，非法字符自动过滤。
...
直接打开 `index.html` 浏览器会因 `file://` 协议限制进入兼容模式（仅音频压缩器不可用，弹奏与转调功能正常）。建议通过本地 HTTP 服务器运行以获得完整体验：
